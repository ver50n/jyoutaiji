  ...
  <div class="subcontainer gallery">
    ...
    @include('components.table.header',[
      'headers' => [
        ...
        'title' => ['sortable' => true, 'title' => trans('common.title')],
        'thumbnail' => ['sortable' => true, 'title' => trans('common.thumbnail')],
        'is_slider' => ['sortable' => true, 'title' => trans('common.is_slider')],
        ...
        <tr>
          <td>
            ...
            <form class="form-grid-delete"
              id="form-grid-delete"
              action="{{ route('manage.gallery.delete', ['id' => $row->id]) }}"
              method="POST"
              style="display: none;">
            @csrf
            </form>
          ...
          <td>{{$row->title}}</td>
          <td><img width="100" src="{{ \App\Utils\FileUtil::getImageUrl('gallery_thumbnail', $row->thumbnail) }}" /></td>
          <td>{{App\Helpers\ApplicationConstant::YES_NO[$row->is_slider]}}</td>
          <td>
...
@section('title')
  @include('layouts.includes.title', ['title' => 'ギャラリー一覧'])
@endsection
